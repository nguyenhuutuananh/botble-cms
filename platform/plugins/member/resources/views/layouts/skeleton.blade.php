  ...
  @if (session()->has('status') || session()->has('success_msg') || session()->has('error_msg') || (isset($errors) && $errors->count() > 0) || isset($error_msg))
    <script type="text/javascript">
      ...
      @if (session()->has('success_msg'))
        noticeMessages.push({'type': 'success', 'message': '{!! session('success_msg') !!}'});
      @endif
      @if (session()->has('status'))
      noticeMessages.push({'type': 'success', 'message': '{!! session('status') !!}'});
      @endif
      @if (session()->has('error_msg'))
        noticeMessages.push({'type': 'error', 'message': '{!! session('error_msg') !!}'});
      @endif
      @if (isset($error_msg))
        noticeMessages.push({'type': 'error', 'message': '{!! $error_msg !!}'});
      @endif
      @if (isset($errors))
        @foreach ($errors->all() as $error)
          noticeMessages.push({'type': 'error', 'message': '{!! $error !!}'});
        @endforeach
      @endif
    </script>
  @endif
