                    ...
                    <div class="app-footer">
                        ...
                        <div class="app-actions">
                            @if (Auth::user()->hasPermission('plugins.edit'))
                                <a href="#" class="btn @if ($plugin->status) btn-warning @else btn-info @endif btn-trigger-change-status" data-plugin="{{ $plugin->path }}" data-status="{{ $plugin->status }}">@if ($plugin->status) {{ trans('packages/plugin-management::plugin.deactivate') }} @else {{ trans('packages/plugin-management::plugin.activate') }} @endif</a>
                            @endif
                            
                            @if (Auth::user()->hasPermission('plugins.remove'))
                            <a href="#" class="btn btn-danger btn-trigger-remove-plugin" data-plugin="{{ $plugin->path }}">{{ trans('packages/plugin-management::plugin.remove') }}</a>
                            @endif
                        </div>
                    </div>
                </div>
